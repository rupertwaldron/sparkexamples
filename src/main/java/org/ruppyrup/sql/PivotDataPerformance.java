 ...
 */
public class PivotDataPerformance {
  ...
  public static void main(String[] args) {
    ...
    sqlspark.conf().set("spark.sql.shuffle.partitions", 4);
...
//                                        "from logging_table group by level, month order by cast(first(date_format(datetime, 'M')) as int), level");
    ...
    Dataset<Row> results1 = dataset.select(
            col("level"),
    ...
    Dataset<Row> results2 = dataset.select(
            col("level"),
            date_format(col("datetime"), "MMMM").alias("month"),
            date_format(col("datetime"), "M").alias("monthnum").cast(DataTypes.IntegerType)
        )
                                ...
                                .count().as("total")
                                .orderBy("monthnum") // if blank do this
                                .drop("monthnum"); // drop will drop any zero rows
